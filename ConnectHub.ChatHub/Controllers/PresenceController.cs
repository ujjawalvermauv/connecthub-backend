...
namespace ConnectHub.ChatHub.Controllers
{
    ...
    public class PresenceController : ControllerBase
    {
        ...
        {
            return Ok(new { userId, isOnline = _presenceService.IsUserOnline(userId) });
        }

        [HttpGet("connection-count/{userId:int}")]
        public IActionResult GetConnectionCount([FromRoute] int userId)
        {
            return Ok(new { userId, connectionCount = _presenceService.GetConnectionCount(userId) });
        }

        [HttpDelete("clear")]
        public IActionResult ClearAll()
        {
            _presenceService.ClearUserConnections();
            return NoContent();
        }
    }
}
